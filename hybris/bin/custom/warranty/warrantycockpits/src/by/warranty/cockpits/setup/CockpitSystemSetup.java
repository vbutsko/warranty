 ...
 */
package by.warranty.cockpits.setup;
...
import by.warranty.cockpits.constants.WarrantyCockpitsConstants;
 ...
 */
@SystemSetup(extension = WarrantyCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(WarrantyCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(WarrantyCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						WarrantyCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/warrantycockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
